    ...
    <header className='sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200 font-sans'>
        <div className='flex items-center justify-between px-4 py-3 md:px-6 max-w-7xl mx-auto'>
            ...
            <div className='flex items-center gap-4'>
               <button  onClick={() => navigate('/profile')} className='p-2 text-gray-600 hover:text-purple-500 transition-colors duration-300 hover:bg-purple-50 rounded-full'>
               ...
               <div className='relative' ref={menuRef}>
                  <button onClick={handleMenuToggle} className='flex items-center gap-2 px-3 py-2 rounded-full cursor-pointer hover:bg-purple-50 transition-colors duration-300 border border-transparent hover:border-purple-200'>
                     <div className='relative'>
                        {user.avatar ? (
                           <img src={user.avatar} alt="avatar" className='w-9 h-9 shadow-sm rounded-full' />
                        ) : (
                  ...
                  {menuOpen && (
                     <ul className='absolute top-14 right-0 w-56 z-50 bg-white border border-purple-100 rounded-2xl shadow-xl overflow-hidden animate-fadeIn'>                        
                           <li className='p-2'>
                                <button onClick={() => { navigate('/profile'); setMenuOpen(false); }} className='w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-purple-50 flex items-center gap-2 group transition-colors duration-300' role='menuitem'>
                                  <Settings className='w-4 h-4 text-gray-700' />
                                  Configurações do Perfil
                                </button>
                           </li>          
                           <li className='p-2'>
                                <button onClick={handleLogout} className='w-full px-3 py-2 text-sm hover:bg-red-50  text-red-600 flex items-center gap-2 '>
